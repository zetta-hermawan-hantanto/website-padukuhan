  ...
  return (
    <div className="bg-[#CCEDEC] flex flex-col mt-16 lg:px-32 lg:py-20 sm:px-24 sm:py-12 px-4 py-3">
      <div className="flex items-center justify-between max-sm:flex-col max-sm:text-center max-sm:gap-4">
        ...
        <Link className="sm:px-6 sm:py-3 px-3 py-2 bg-white font-bold rounded-full shadow-md hover:bg-white/75" href="/profil-bolang">
          Lebih Lanjut
        </Link>
      </div>
      <div className="flex justify-center gap-10 mt-12 max-xl:flex-col">
        {potentionalData.map((data, index) => (
          <CardPotentional key={index} title={data.title} description={data.description} image={data.image} />
        ))}
      </div>
    </div>
  );
